...
function readAsciiHeaders(file) {
  ...
  const parseItentifierByte = (fileMapping, currentByte) => {
    if (!isAsciiNewLine(currentByte)) {
      return {
        ...fileMapping,
        typeIdentifier:
          fileMapping.typeIdentifier + String.fromCharCode(currentByte),
      };
    } else {
  ...
  const parseEndiannessByte = (fileMapping, currentByte) => {
    if (!isAsciiNewLine(currentByte)) {
      return {
        ...fileMapping,
        endianness: fileMapping.endianness + String.fromCharCode(currentByte),
      };
    } else {
  ...
  const parseHeadersByte = (fileMapping, currentByte) => {
    if (isAsciiComma(currentByte) || isAsciiNewLine(currentByte)) {
      ...
      return {
        ...fileMapping,
        currentFieldHeader:
          fileMapping.currentFieldHeader + String.fromCharCode(currentByte),
      };
    }
  };
  ...
  const parseFilePrecisionsByte = (fileMapping, currentByte, index) => {
    if (isAsciiComma(currentByte) || isAsciiNewLine(currentByte)) {
      ...
      if (fileMapping.currentFieldPrecision !== "PRECISION")
        throw new Error("Error. Expected field header precision values.");
      return {
        ...fileMapping,
        currentFieldPrecision: "",
      };
      ...
      return {
        ...fileMapping,
        currentFieldPrecision:
          fileMapping.currentFieldPrecision + String.fromCharCode(currentByte),
      };
    }
  };
  let first = true;
    ...
    (fileMapping, currentByte, idx) => {
      switch (fileMapping.currentlyParsing) {
        ...
        case FileFields.FILE_PRECISIONS:
          return parseFilePrecisionsByte(fileMapping, currentByte, idx);
        default:
