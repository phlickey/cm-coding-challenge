...
import { PossibleEndiannessValues, readAsciiHeaders } from './readAsciiHeaderInfo'
...
const readBinaryData = (file) => {
    ...
    const endianness = asciiHeaders.endianness === PossibleEndiannessValues.LITTLE_ENDIAN
    ...
    for (let i = 0; i < asciiHeaders.fieldHeaders.length; i++) {
      ...
      typeSet[asciiHeaders.fieldHeaders[i]] = getjBinaryDataTypeFromPrecision({fieldPrecision, endianness})
    }
    ...
    do {
      try {
        ...
        if (boundry !== "#")
          throw new Error("expected sample delinator at beginning of new sample");
        let dataPoint = {};
        ...
        if  (e.message==="expected sample delinator at beginning of new sample") continue; // try to resync by polling each byte until the boundry char is found
        
      }
    } while (bin.view.byteOffset <= bin.view.byteLength)
    return { ...asciiHeaders, countSamples: trip.length, trip, remainingData: "" }
  };
