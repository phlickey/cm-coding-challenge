...
import { getTripTimeInfo, parseDateFields } from '../utils/parse-trips/parseTripData'
...
export const FileInput = ({dispatchError, dispatchResolveError, dispatchFilesLoaded, isErrorState}) => {
  const onDrop = useCallback(async (acceptedFiles) => {
    try {
        let parsedDataFiles = await Promise.all(acceptedFiles.map(async (file) => {
  ...
  return (
    <StyledContainer {...getRootProps()}>
      ...
      {isDragActive ? (
        ...
        <p>Drop one or more .bin trip files here, or click to choose...</p>
      )}
    </StyledContainer>
  );
}
...
const mapDispatchToProps = dispatch => ({
    ...
    dispatchFilesLoaded: ({numberOfFiles, tripStartTime, tripEndTime, tripDurationInMinutes, tripData, fieldHeaders}) => dispatch(getLoadFilesAction({numberOfFiles, tripStartTime, tripEndTime, tripDurationInMinutes, tripData, fieldHeaders})),
    dispatchResolveError: ()=>dispatch(resolveErrorActionCreator())
})

export const ConnectedFileInput = connect(mapStateToProps, mapDispatchToProps)(FileInput)