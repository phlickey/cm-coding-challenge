...
export const Table = ({ displayTable, headings, tripDataPoints }) => {
    ...
    const sortBy = (heading) => {
        updateFilterDirs({
            ...filterDirs,
            [heading]:
                filterDirs[heading] === FilterModes.ASC
                    ? FilterModes.DESC
                    : FilterModes.ASC,
        })
        const sorted = tripDataPoints
            .sort((pointA, pointB) => {
                let a
                let b
                                        ...
                                        {(() => {
                                            switch (typeof val) {
                                                ...
                                                case 'string':
                                                    return val
                                                case 'object':
                                                    if (val.toLocaleDateString)
                                                        return `${val.toLocaleTimeString()}  ${val.toLocaleDateString()}`
...
const mapStateToProps = (state) => ({
    displayTable:
        ...
        state.currentViewMode === ViewModes.TABLE,
